...
const char PatternPattern3[] PROGMEM ="User pattern 3";
...
const char * const PatternNamesArray[] PROGMEM = { PatternNone, PatternRainbowCycle, PatternColorWipe, PatternFade, PatternDelay,
        ...
        PatternPattern2, PatternPattern3
...
bool NeoPatterns::update() {
    if ((millis() - lastUpdate) > Interval) {
        ...
        case PATTERN_USER_PATTERN2:
            ...
            break;
        case PATTERN_USER_PATTERN3:
            tPatternEnded = Pattern3Update();
            break;
#if !defined(DO_NOT_USE_MATH_PATTERNS)
...
void __attribute__((weak)) UserPattern3(NeoPatterns *aNeoPatterns, color32_t aColor, uint16_t aIntervalMillis,
        uint16_t aRepetitions, uint8_t aDirection) {
    /*
     * Sample implementation
     */
 ...
 * The user may specify its own implementation
 */
bool __attribute__((weak)) UserPattern3Update(NeoPatterns *aNeoPatterns, bool aDoUpdate) {
    if (aDoUpdate) {
        // clear old pixel
        aNeoPatterns->setPixelColor(aNeoPatterns->Index, COLOR32_BLACK);
...
bool NeoPatterns::Pattern3Update(bool aDoUpdate) {
    return UserPattern3Update(this, aDoUpdate);
}
/*****************************************************************
